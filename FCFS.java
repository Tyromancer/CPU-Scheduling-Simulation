        	...
        	if(running != Process.EMPTY)
        	{
        		if(running.tick())
        		{
					...
					case RUNNING:
						...
						System.out.println(String.format("time %dms: Process %s switching out of CPU; will block on I/O until time %dms %s", time, running.id(), running.getIOTime(), queueInfo()));
        				break;
